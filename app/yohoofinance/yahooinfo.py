...
def build_combined_trend_section(info, stock_hist, index_hist):
    ...
    try:
        ...
        stock_view = stock_df.tail(30)
        index_view = index_df.tail(30) if not index_df.empty else pd.DataFrame()
        
        # Charts
        stock_chart = generate_mini_candlestick(stock_view, width=380, height=180)
        index_chart = generate_mini_candlestick(index_view, width=380, height=180) if not index_view.empty else ""
...
def fetch_info2(symbol):
    try:
        ...
        # ADDED: Daily Trend Overview with SVG Chart and Insights
        daily_trend = build_daily_trend_section(info, hist)
        if daily_trend:
            parts.append(daily_trend)
