...
SHORT_NAMES = {
    ...
    "volumeSpikeTrend":"Vol Spike Trend"
}
...
PRICE_VOLUME_GROUPS = {
    ...
    "Volume & Liquidity": ["Vol","Avg Vol 10D","Avg Vol 3M","Vol Spike","Vol Spike Trend"],
...
def format_value(k, v):
    ...
    if isinstance(v,(int,float)):
        ...
        if "percent" in lk or "spike" in lk:
...
def build_volume_spikes(info, hist_df):
    ...
    if today_vol and not hist_df.empty and len(hist_df) >= 10:
        ...
        if avg_10d > 0:
            out["volumeSpike"] = (today_vol / avg_10d - 1) * 100
...
def fetch_info(symbol):
    ...
    try:
        ...
        if pv: html+=html_card(f"{MAIN_ICONS['Price / Volume']} Price / Volume",build_price_volume_section(info,pv,hist))
        # Fundamentals
