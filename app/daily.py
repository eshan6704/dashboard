...
def fetch_daily(symbol, date_end, date_start):
    try:
        ...
        if df.empty:
            return f"<h3>No data found for {symbol} in this range.</h3>"

        # Flatten MultiIndex if present
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)

        # Reset index and convert numeric
        df = df.reset_index()
        ...
        if df.empty:
            return f"<h3>No valid numeric data for {symbol} after cleaning.</h3>"
        ...
        # -------------------------
        # Limit view for charts
        # -------------------------
        view = df.tail(120)
        if view.empty:
            return f"<h3>No data for {symbol} to render charts.</h3>"
...
<h2>{symbol} – Professional Daily Dashboard</h2>
