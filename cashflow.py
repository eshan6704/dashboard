...
def fetch_cashflow(symbol):
    ...
    try:
        ...
        df = ticker.cashflow
        if df.empty:
            content_html = f"<h1>No cash flow data available for {symbol}</h1>"
        else:
            # Format numeric columns
            for col in df.columns:
                if ptypes.is_numeric_dtype(df[col]):
                    df[col] = df[col].apply(format_large_number)
            content_html = f"<div class='card'><h2>Cash Flow</h2>{df.to_html(classes='styled-table', border=0)}</div>"
...
<html>
<head>
    <title>Cash Flow for {symbol}</title>
    {STYLE_BLOCK}
</head>
<body>
    {content_html}
